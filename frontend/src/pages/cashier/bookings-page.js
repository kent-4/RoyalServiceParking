...
function renderActionCell(booking) {
  ...
  if (booking.canOpenPayment) {
    actions.push('<span class="table-helper-text">Payment page is the next rebuild slice.</span>');
  }

  if (booking.canViewReceipt) {
    actions.push('<span class="table-helper-text">Receipt view follows the payment flow rebuild.</span>');
  }
          ...
          ${renderPanelCard({
            ...
            content: `
              <ul class="journey-list">
                ...
                <li>Payment and receipt routes remain the next rebuild slice after this operational table.</li>
              </ul>
            `
          })}
        </section>
          ...
          if (alertsRoot) {
            alertsRoot.innerHTML = currentResponse.arrivedCount > 0
              ? renderInlineAlert({
                  ...
                  message: `${currentResponse.arrivedCount} arrived booking${currentResponse.arrivedCount === 1 ? "" : "s"} are ready for the upcoming payment-flow rebuild.`
                })
              : "";
          }
          renderSummary();
