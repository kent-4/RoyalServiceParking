...
Notes / blockers: `Phase 2 public/auth routes are now implemented in the JavaScript frontend around one shared /login route, with legacy role-specific login URLs reduced to compatibility redirects back to /login rather than distinct public login experiences. Registration, verification result handling, forgot-password request, reset-token validation, and reset-password success states are all live. A follow-up public copy pass also removed cashier/admin promotion from the public navbar, footer, landing page, and shared login page so the customer journey stays front-and-center while backend authorization still resolves the correct post-login destination. End-to-end email delivery and token lifecycle still depend on backend runtime configuration and manual verification outside the frontend build checks.`
